...
// Get project details for chat header
export async function getProjectForChat(projectSlug: string) {
    try {
        const session = await auth()
        if (!session?.user?.id) {
            throw new Error("Unauthorized")
        ...
        // Basic access check - detailed check is done in getProjectMessages
        return {
            success: true,
            project
        }
    } catch (error) {
        console.error("Get project for chat error:", error)
        return {
            success: false,
