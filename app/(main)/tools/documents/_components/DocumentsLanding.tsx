                        ...
                        <CardContent className="p-0">
                            <div className="p-6 space-y-4">
                                ...
                                <div className="space-y-1">
                                    <h3 className="font-black text-lg tracking-tighter truncate uppercase italic">
                                        {doc.title}
                                    </h3>
                                    <p className="text-[10px] font-mono font-bold text-neutral-400 uppercase tracking-widest truncate">
                                        Ref: {doc.id.slice(-8)}
                                    </p>
                                </div>
                            ...
                            <div className="px-6 py-4 bg-neutral-50 dark:bg-neutral-900/50 border-t border-neutral-200 dark:border-neutral-800 flex items-center justify-between">
                                <div className="flex flex-col">
                                    <span className="text-[8px] font-mono text-neutral-400 uppercase tracking-widest">Modified_At</span>
                                    <span className="text-[10px] font-bold">{format(new Date(doc.updatedAt), 'yyyy.MM.dd')}</span>
                                </div>
                                <div className="flex gap-2">
                                    <Button size="icon" variant="outline" className="h-9 w-9 rounded-xl border-neutral-200 dark:border-neutral-800" asChild>
                                        <a href={doc.fileUrl} target="_blank" rel="noopener noreferrer">
                                            <Download className="h-4 w-4" />
                                        </a>
                                    </Button>
                                    <Button size="icon" className="h-9 w-9 rounded-xl bg-black dark:bg-white text-white dark:text-black hover:scale-110 transition-transform" asChild>
                                        <Link href={`/tools/documents/${doc.category.toLowerCase()}/${doc.id}`}>
            ...
            {categoryDocs.length === 0 && (
                <div className="col-span-full py-20 text-center border-2 border-dashed border-neutral-200 dark:border-neutral-800 rounded-3xl">
                    <div className="flex flex-col items-center gap-4 opacity-20">
                        ...
                        <div className="space-y-1">
                            <p className="font-black tracking-tighter uppercase italic">Empty_Archive</p>
                            <p className="text-[10px] font-mono uppercase tracking-widest">Waiting for data synchronization...</p>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="space-y-1">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono tracking-widest text-neutral-500 uppercase">System_Unit // Archive_Management</span>
                    </div>
                    <h1 className="text-4xl font-black tracking-tighter uppercase italic">Document_Terminal</h1>
                    <p className="text-neutral-500 text-sm max-w-md font-medium">Protocol for secure document storage, indexing, and neural analysis of corporate assets.</p>
                </div>
                    ...
                    <TabsList className="bg-neutral-100 dark:bg-neutral-900 p-1 h-auto rounded-2xl flex-wrap">
                        {categories.map(cat => (
                            <TabsTrigger
                                key={cat.id}
                                value={cat.id}
                                className="rounded-xl px-6 py-2.5 text-xs font-bold uppercase tracking-widest data-[state=active]:bg-black data-[state=active]:text-white dark:data-[state=active]:bg-white dark:data-[state=active]:text-black transition-all"
                            >
                                <div className="flex items-center gap-2">
                                    {cat.icon}
                                    {cat.label}
